...
gROOT->ForceStyle();
  ...
  double avJw = 0.;
  double Efermi40 = -4.71;
  double Cshift40 = 0.759*1.7*20./1.22/pow(40.,1./3.);
  for (int i=0;i<N;i++)
    {
      ...
      getline(file,name);
      cout << E << " " << jw << endl;
      x[i] = E*40./41. - Efermi40;
      xx[i] = E*40./41.- Cshift40;
      y[i] = jw;
      yy[i] = jr;
      avJw += jw;
    }
  ...
  TGraph gpca40(N,x,y);
  gpca40.SetMarkerStyle(20);
  gpca40.SetMarkerColor(3);
  gpca40.SetLineColor(3);
  ...
  cout << NP << " " << ii << endl;
  avJw = 0.;
  double Efermi48 = -13.21;
  double Cshift48 = 0.759*1.7*20./1.22/pow(48.,1./3.);
  cout << " Cshift= " << Cshift48 << endl;
  for (int i=0;i<NP;i++)
    {
     
      file >> E >> jr >> jw >> vrms >> wrms >> jso >> sorms;
      getline(file,name);
      xp[i] = E*48./49. - Efermi48;
      xxp[i] = E*48./49. - Cshift40;
      yp[i] = jw;
      yyp[i] = jr;
      avJw += jw;
    }
  ...
  text.SetNDC();
  text.SetTextColor(3);
  text.DrawLatex(.7,.8,"p+^{40}Ca");
  text.SetTextColor(4);
  text.DrawLatex(.7,.7,"n+^{40}Ca");
  ...
  line.SetLineColor(4);
  line.DrawLine(10.86,0,10.86,200);
  ...
  for (int jj=0;jj<PP;jj++) xdomP[jj] += Efermi48 - Cshift48; 
  TGraph ggvP(PP,xdomP,vdomP);
  ggvP.SetLineColor(4);
  ggvP.SetLineWidth(1.);
  ggvP.Draw("L");

  for (int jj=0;jj<NN;jj++) xdom[jj] += Efermi40 - Cshift40; 
