...
gROOT->ForceStyle();
  ...
  avJw = 0.;
  double EfermiP = -4.71;
  double Cshift = 0.759*1.7*20./1.22/pow(40.,1./3.);
  ...
  for (int i=0;i<NP;i++)
    {
      ...
      file >> E >> jr >> jw >> vrms >> wrms >> jso >> sorms;
      getline(file,name);
      xp[i] = E*40/41. - EfermiP;
      xxp[i] = E*40./41. - Cshift;
      yp[i] = jw;
      yyp[i] = jr;
      avJw += jw; 
    }
