...
double ymax = 1100.;
 ...
 text.SetNDC();
 text.SetTextSize(.1);
 text.DrawLatex(.5,.35,"(b) p+^{40}Ca");

 ifstream file("pca40r.dat");
 int Npoints;
 float x[300];
 float y[300];
 float error[300];
...
//***************************************************************
 ...
 frame2->Draw();
 text.DrawLatex(.5,.75,"(a) n+^{40}Ca");
text.SetTextColor(4);
text.DrawLatex(.8,.45,"#sigma_{tot}");
text.SetTextColor(2);
text.DrawLatex(.3,.48,"#sigma_{react}");
 file.open("nca40r.dat");
 int NNpoints;
