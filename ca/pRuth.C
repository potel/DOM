...
gROOT->ForceStyle();
 ...
 TCanvas canvas("pRuthCa","",900,600);

//divide up canvas into three pads
 double overlap = .1;
 TPad *pad1  = new TPad("pad1","",0.,0.,.3333+2.*overlap/3.,1.);
 TPad *pad2  = new TPad("pad2","",.3333-overlap/3.,0.,.666+overlap/3.,1.);
 TPad *pad3  = new TPad("pad3","",.666-2.*overlap/3.,0.,1.,1.);
...
double ymax = 1300.;
 ...
 pad1->cd();
   pad1->SetLogy();
   TH2S frame ("frame","",10,0,180,10,.1,1.e25);
 ...
 text.SetNDC();
 text.SetTextSize(.08);
 text.DrawLatex(.68,.8,"p+^{40}Ca");

 ifstream fxdata;
 fxdata.open("../ca/pca40xx.dat");
...
//**********************************************************
 pad3->cd();
   pad3->SetLogy();
   TH2S frame2 ("frame2","",10,0,180,10,.1,1.e25);
 ...
 text.DrawLatex(.68,.8,"p+^{48}Ca");

 fxdata.open("../ca/pca48xx.dat");
 ...
 for (;;)
   {
    ...
    if (Elab >= 10.)
       {
	 for (int i=0;i<Ndata;i++) y[i] *=2.;
	 for (int i=0;i<NthTry;i++) yy[i] *=2.;
       }
    ...
    if (Elab >= 100.)
       {
	 for (int i=0;i<Ndata;i++) y[i] *=scale*2.;
	 for (int i=0;i<NthTry;i++) yy[i] *=scale*2.;
	 scale *= scaleInc;
...
//*********************************************************************
 text.DrawLatex(.68,.89,"p+^{44}Ca");

 fxdata.open("../ca/pca44xx.dat");

 scale = scaleInc*1000000000000000000000;
 graphE.SetMarkerStyle(20);
 graphE.SetMarkerSize(.5);
 bool first = 1;
 for (;;)
   {
	 ...
	 for (int i=0;i<Ndata;i++) y[i] *=1000000000000.;
	 for (int i=0;i<NthTry;i++) yy[i] *=1000000000000.;
...
fxdata.clear();
 ...
 text.SetTextSize(.05);
   ...
   pad3.cd();
   text.SetTextSize(.04);
   text.DrawLatex(.4,.16,"E_{lab}<10 MeV");  
   text.SetTextColor(2);
   text.DrawLatex(.4,.39,"10<E_{lab}<20 MeV");  

   text.SetTextColor(3);
   text.DrawLatex(.4,.63,"20<E_{lab}<40 MeV");  

   text.SetTextColor(4);
   text.DrawLatex(.4,.69,"40<E_{lab}<100 MeV"); 

   pad3->cd();
   text.SetTextColor(6);
   text.DrawLatex(.5,.89,"E_{lab}>100 MeV"); 
