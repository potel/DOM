...
reaction::reaction(string *title0,bool flag0)
{
  ...
  scatter->ReadRealPot();                           /* reading ee'p pot */
  scatter->ReadImagPot();                           /* reading ee'p pot */
  if (flag == 0) return;
...
void reaction::partialWave(){
   double Elab = 100;
   double Ecm = energyLab2Cm(Elab);

   InitializeForEcm(Ecm,Elab);
   cout<<"Kwave now = "<<scatter->Kwave<<endl;
   //scatter->integrateWave();
   //scatter->integrateWaveOptso();
   //scatter->integrateWaveCseq();
   //scatter->integrateWaveCseqDOM();
   scatter->integrateWaveSch();
 ...
 *--------------------------------------------------------------------------------------
 *       Class:  reaction
 *      Method:  reaction :: Plot
 * Description:  Runs the plot method in scat
 *--------------------------------------------------------------------------------------
 */
void reaction::Plot()
{
   ...
   scatter->Plot();
   return ;
}		/* -----  end of method reaction::Plot  ----- */


void reaction::boundWave(double& Elower,double Eupper, double j, int l, int Ifine){
   bound(Elower,Eupper,j,l,Ifine);
   cout<<"Ehole = "<<Elower<<endl;
   InitializeForEcm(Elower,0);
...
bool reaction::AdjustFermi()
{
  ...
  double VHF_old = 0.;
  cout<<"Before AdjustFermi: VHF = "<<VHF<<endl;
  for (;;)
    {


      double Efermi_calculate = FindFermi();
      if (fabs(Efermi-Efermi_calculate) < .04){
         cout<<"After AdjustFermi: VHF = "<<VHF<<endl;
         return 1;
      }
